module codeBase {
    export class DefaultDecoder implements IDecoder {
        ...
        public decode(bytePacket: egret.ByteArray): Packet {
            var messageId: number = this.decodeMessageId(bytePacket);
            var packet: Packet = PacketFactory.createPacket(messageId, false);
            if (packet && bytePacket.bytesAvailable > 0) this.decodeHeader(bytePacket, packet);
            if (packet && bytePacket.bytesAvailable > 0) this.decodeBody(bytePacket, packet);
            return packet;
        }
        private decodeMessageId(bytePacket: egret.ByteArray): number {
            var messageId: number = bytePacket.readUnsignedShort();
            bytePacket.position = 2;
            return messageId;
        }
        public decodeHeader(bytePacket: egret.ByteArray, packet: Packet): void {
        ...
        public readByteArray(defItem: any, bytePacket: egret.ByteArray, target: any): void {
            ...
            var data: egret.ByteArray = ObjectPool.getByClass(egret.ByteArray);
            if (length > 0) {
