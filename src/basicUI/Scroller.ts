module codeBase {
	...
	*/
	export class Scroller extends BasicGroup implements ILayout {
		...
		protected onTouch(e: TouchEvent) {
			let s = this;
			switch (e.type) {
				case BasicUIEvent.TOUCH_BEGIN:
					s.stage.addEventListener(BasicUIEvent.TOUCH_END, s.onTouch, s);
					s.stage.addEventListener(BasicUIEvent.TOUCH_MOVE, s.onTouch, s);
					s.stPos.x = e.stageX;
					s.stPos.y = e.stageY;
				...
				case BasicUIEvent.TOUCH_END:
					s.stage.removeEventListener(BasicUIEvent.TOUCH_END, s.onTouch, s);
					s.stage.removeEventListener(BasicUIEvent.TOUCH_MOVE, s.onTouch, s);
					s.hideShow(0, 100);
		...
		protected onMouseMoveEvent(ev: TouchEvent) {
			let s = this;
			//console.log(ev.type);
