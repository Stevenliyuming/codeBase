module codeBase {
	...
	export class ListGroup extends BasicGroup implements ILayout {
		...
		public addItem(item: DisplayObject): void {
			...
			item.addEventListener(BasicUIEvent.TOUCH_BEGIN, s.onTouch, s);
			item.addEventListener(BasicUIEvent.TOUCH_END, s.onTouch, s);
			s.items.push(item);
		}
		...
		public removeItem(item: DisplayObject): void {
			...
			if (s.contentView.contains(item)) {
				...
				item.removeEventListener(BasicUIEvent.TOUCH_BEGIN, s.onTouch, s);
				item.removeEventListener(BasicUIEvent.TOUCH_END, s.onTouch, s);
