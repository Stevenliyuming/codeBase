module codeBase {
 	export class Skeleton extends egret.Sprite {
		private static _inited: boolean = false;
		private static worldClock: dragonBones.WorldClock;
		private static _mTime: number = 0;
		private static _gapTime: number = 0;
		...
		public _armature: dragonBones.EgretArmatureDisplay;
		...
		public setDataByName(bName: string, skPath: string = "", boneName: string = null, armatureName: string = null, movieClipScale: number = 1): void {
			...
			let forceSk: boolean;
			let pathStr;
			if (GlobalSetting.deviceVer === "canvas") {
				pathStr = skPath.replace("skeleton", "framesAnimation");
				forceSk = !RES.hasRes(pathStr + bName + "_mc.json");
			}
			if (GlobalSetting.deviceVer === "webgl" || forceSk) {
				//使用骨骼动画
				s.setData(
					RES.getRes(skPath + bName + "_ske_json"),
					RES.getRes(skPath + bName + "_tex_json"),
					RES.getRes(skPath + bName + "_tex_png"), boneName ? boneName : bName, armatureName
				);
			}
			else {
				//使用帧动画			
				s.setMovieClipData(
					RES.getRes(pathStr + bName + "_mc_json"),
					RES.getRes(pathStr + bName + "_tex2_png"), boneName ? boneName : bName, 1/movieClipScale);
			}
		}
		...
		*/
		...
		private clear(remove: boolean = false): void {
			...
			if (s._sound) {
				SoundManager.Instance.stopSoundByName(s._sound);
			}
			s._textureData = null;
			s._texture = null;
			s._display = null;
		...
		public gotoAndPlay(actionName: string, playTimes: number = 1, sound: string = null, endAct: string = null, soundEndCall: Function = null, soundThisObject: any = null): void {
			...
			if (s._sound) {
				SoundManager.Instance.PlaySoundByName(s._sound, 1, s.soundEnd, s, 1, true, false);
			}
		}
