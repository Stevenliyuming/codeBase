module codeBase{
    export class MouseJointHelper extends egret.EventDispatcher {
        ...
        public constructor(stageRef: egret.Stage, pSceneCtn: egret.DisplayObjectContainer, pWorld: p2.World, hasDisplaySkin: Boolean = true) {
            ...
            if (hasDisplaySkin) {//是否有显示皮肤
                this.mouseBody = easyP2.P2Space.addOneBox(this.world, this.sceneCtn, 400, 400, 20, 20, 0, p2.Body.KINEMATIC);//mouseBody
            ...
            for (var i = 0; i < this.sceneCtn.numChildren; i++) {
                ...
                if (disp.hitTestPoint(e.stageX, e.stageY, true)) {
                    ...
                    if (temBody.type == p2.Body.DYNAMIC) {
                        ...
                        var p2x: number = easyP2.P2Space.convertEgretValueToP2(e.stageX);
                        var p2y: number = easyP2.P2Space.convertEgretY_To_P2Y(e.stageY);
