    ...
    */
    var Scroller = (function (_super) {
        ...
        function Scroller(w, h, content, align, sliderSKin, barVisible) {
            ...
            s.height = h;
            s.touchEnabled = true;
            s.addEventListener(codeBase.BasicUIEvent.TOUCH_BEGIN, s.onTouch, s);
            s.startPos = new codeBase.Point;
            s.stPos = new codeBase.Point;
            s._contentPos = new codeBase.Point;
            s.content = content;
            s.alignType = align;
        ...
        Scroller.prototype.onTouch = function (e) {
            ...
            switch (e.type) {
                case codeBase.BasicUIEvent.TOUCH_BEGIN:
                    s.stage.addEventListener(codeBase.BasicUIEvent.TOUCH_END, s.onTouch, s);
                    s.stage.addEventListener(codeBase.BasicUIEvent.TOUCH_MOVE, s.onTouch, s);
                    s.stPos.x = e.stageX;
                    s.stPos.y = e.stageY;
                ...
                case codeBase.BasicUIEvent.TOUCH_END:
                    s.stage.removeEventListener(codeBase.BasicUIEvent.TOUCH_END, s.onTouch, s);
                    s.stage.removeEventListener(codeBase.BasicUIEvent.TOUCH_MOVE, s.onTouch, s);
                    s.hideShow(0, 100);
        ...
        Scroller.prototype.timeMove = function (x, y) {
            ...
            if (time < 500) {
                ...
                var tw = codeBase.Tween.get(target, {
                ...
                if (this.alignType == codeBase.Style.VERTICAL) {
                    ...
                    if (h < 0 && target.y + value < (s.viewPort.height - s._content.height))
                        h = s.viewPort.height - s._content.height; //向上滑动
                    ...
                    if (w < 0 && target.x + value < (s.viewPort.width - s._content.width))
                        w = s.viewPort.width - s._content.width; //向左滑动
        ...
        Scroller.prototype.setScrollRect = function () {
            var s = this;
            if (s._content) {
                s._content.scrollRect = new codeBase.Rectangle(0, 0, s.width, s.height);
            }
        };
